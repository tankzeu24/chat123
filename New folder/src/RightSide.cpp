...
const bool RightSide::empty() const
{
	...
	else
	{
		...
		{
			for   (auto terminal : this->elements)
			{
...
{
    //Compare RHS to the current set ,if the RHS is empty will return true
    /*
     cout<<"current rightside is : ("<<toString()<<" ) "<<endl;
...
const RightSide RightSide::replaceBeginning(const RightSide & newBeginning) const
{
	if (this->empty())
	{
	  //  cout<<"I will not change the top of stack which is "<<newBeginning.toString()<<endl;
		return newBeginning;

	}
   // cout<<"Passed newbeginning = " <<newBeginning.toString()<<endl;
    //cout<<" current copyElements = "<<elements.toString()<<endl;
   ...
   // std::cout << toString()<<endl;
	...
	copyElements.insert(copyElements.begin(), newBeginning.elements.begin(), newBeginning.elements.end()); // replace start with newstart
	...
	for   (auto element : copyElements)
	{
		if (element->isTerminal())
		{
			...
			//cout<<"I will add"<<element->toString()[0]<<endl;
		}
	...
	//cout<<"I will change ( "<<newBeginning.toString()<< " )  with ( " <<result.toString()<< " )"<<endl;
	//cout<<"------"<<endl;
